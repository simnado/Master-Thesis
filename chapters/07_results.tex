...
Das aktuelle Baseline-Modell bilden in seiner ursprünglichen Form mit $T=32$ und $\tau = 2$ eine Clip-Dauer von 2.67 Sekunden ab.
...
Das vorgehen ist weiter motiviert durch die Mindest-Clip-Dauer von 4 Sekunden in SoccerDB \bzw der unteren Grenze von 3 Sekunden basierend auf \gls{sbod}.

Alle Modelle nutzen eine dynamisches Avg-\pool-Layer zwischen dem letzten \conv- und dem ersten \fc-Layer, das die Verarbeitung von Clips variabler Länge $T$ zulässt.
Um eine geeignete Methode zu finden, wird der Einfluss des Avg-Poolings durch ein höheres $T$ und von schnelleren Clips durch einen höheren Zeitschritt $\tau$ verglichen.
Die Grid-Suche betrachtet also Punkte für verschiedene Werte von $T$ und $\tau$, wobei die dadurch beeinflusste Clip-Dauer sich innerhalb der unteren und oberen Grenze von 3 \bzw 6 Sekunden bewegen soll.
...
Die Punkte für $\tau$ werden so gewählt, dass sich die resultierende Clip-Dauer etwas um eine Sekunde unterscheidet.
Zusätzlich wird das Baseline-Modell der vorherigen Phase erneut getestet.
\autoref{fig:exp-hparams-grid} zeigt alle getesteten Punkte der Grid-Suche, sowie Orientierungslinien für die resultierende Clip-Dauer.
...
Eine erhöhte Clip-Dauer wirkt sich ebenfalls positiv auf den Recall auf.
Die Precision scheint hingegen unabhängig von der Clip-Dauer zu sein.
