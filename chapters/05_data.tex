...
\begin{table}
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        table head=\hline,late after line=\\\hline]{tbl/actions_a.csv}
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \csvreader[no head,tabular=|l|r|r|r|,
        table head=\hline,late after line=\\\hline]{tbl/actions_b.csv}
        {1=\colName,2=\colCount,3=\colAvg,4=\colMax}
        {\colName & \colCount & \colAvg & \colMax}
    \end{subfigure}
...
\begin{enumerate}
    ...
    \item \code{badBehavior} mit \code{card}: 61 \%
    \item \code{penaltyKick} mit \code{goal}: 59 \%
    \item \code{goal} mit \code{footShot}: 57 \%
    \item \code{collected} mit \code{cross}: 43 \%
\end{enumerate}
...
\begin{figure}
    \centering
    \bigimage{fig/data-pipeline}{0.6\textwidth}
...
\begin{figure}
    ...
    \end{subfigure}
    \caption[Beispielhafte Data-Augmentation eines Samples]{Beispielhafte Data-Augmentation eines Samples der Klasse \code{corner} bestehend aus $T=48$ Frames (Quelle: Eigene Darstellung)}
